...
class BlueCubeScript extends Script {
  ...
  zıpla(other,zıpla) {
    if (other != "ground"){
      this.gameObjects[other].transform.translation = translate(zıpla *2, 1, 9);
...
class Hierarchy {
  constructor(gl) {
    ...
    const script2 = new BlueCubeScript();
    script2.gameObject = gameObjects["fixedCube2"];
    script2.gameObjects = this.gameObjects;
    gameObjects["fixedCube2"].component.script = script2;
    ...
    gameObjects["fixedCub5"] = new Cube(
      ...
      new Transform({ translation: translate(-1, 0, 4) })
    );
...
class GameObject {
  ...
  initAttributeBuffers() {
    ...
    u_Clicked = this.gl.getUniformLocation(
      this.program,
      "clicked"
    );
